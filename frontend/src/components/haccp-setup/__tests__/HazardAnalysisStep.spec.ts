...
vi.mock('@/components/ui/select/SelectTrigger.vue', () => ({ default: defineComponent({ setup(_, { slots }) { return () => h('div', slots.default?.()) } }) }))
vi.mock('@/components/ui/select/SelectValue.vue', () => ({ default: defineComponent({ setup(_, { slots }) { return () => h('span', slots.default?.()) } }) }))
vi.mock('@/components/ui/select/SelectContent.vue', () => ({ default: defineComponent({ setup(_, { slots }) { return () => h('div', slots.default?.()) } }) }))
...
describe('HazardAnalysisStep', () => {
  it('shows KKP summary and risk labels from current entries', () => {
    const wizard = {
      hazardEntries: [
        { id: '1', processStepName: 'Mottak', hazardType: 'BIOLOGICAL', hazardDescription: '', likelihood: 3, severity: 2, preventiveMeasure: '' },
        { id: '2', processStepName: 'Lagring', hazardType: 'CHEMICAL', hazardDescription: '', likelihood: 1, severity: 2, preventiveMeasure: '' },
      ],
    }
  ...
  it('updates textareas and select values', async () => {
    const wizard = {
      hazardEntries: [
        { id: '1', processStepName: 'Mottak', hazardType: 'BIOLOGICAL', hazardDescription: '', likelihood: 1, severity: 1, preventiveMeasure: '' },
      ],
    }
